...
std::vector<HANDLE> GetBloxxiaProcesses(bool include_client = true, bool include_studio = true)
{
	std::vector<HANDLE> result;
	if (include_client)
	{
		for (HANDLE handle : ProcUtil::GetProcessesByImageName("BloxxiaPlayerBeta.exe")) result.emplace_back(handle);
	}
	if (include_studio) for (HANDLE handle : ProcUtil::GetProcessesByImageName("BloxxiaStudioBeta.exe")) result.emplace_back(handle);
	return result;
}

HANDLE GetBloxxiaProcess()
{
	auto processes = GetBloxxiaProcesses();
...
const void *FindTaskScheduler(HANDLE process, const char **error = nullptr)
{
	try
	{
		...
		while (true)
		{
			...
			{
				if (error) *error = "Failed to get process base! Please restart Bloxxia FPS Unlocker.";
				return nullptr;
			}
		}
...
struct BloxxiaProcess
{
	...
	void Tick()
	{
		...
		if (!ts_ptr)
		{
			...
			if (!ts_ptr)
			{
				...
				if (retries_left-- <= 0)
					NotifyError("bloxxiafpsunlocker Error", error ? error : "Unable to find TaskScheduler! This is probably due to a Bloxxia update-- watch the github for any patches or a fix.");
				return;
			}
		}
		...
		if (ts_ptr && !fd_ptr)
		{
			try
			{
				if (auto scheduler = (const uint8_t *)(ProcUtil::ReadPointer(handle, ts_ptr)))
				{
					...
					{
						if (retries_left-- <= 0)
							NotifyError("bloxxiafpsunlocker Error", "Variable scan failed! This is probably due to a Bloxxia update-- watch the github for any patches or a fix.");
						return;
					}
			...
			{
				printf("[%p] BloxxiaProcess::Tick failed: %s (%d)\n", handle, e.what(), e.GetLastError());
				if (retries_left-- <= 0)
					NotifyError("bloxxiafpsunlocker Error", "An exception occurred while performing the variable scan.");
			}
		}
	}
	...
	void SetFPSCap(double cap)
	{
		if (fd_ptr)
		{
			...
			{
				printf("[%p] BloxxiaProcess::SetFPSCap failed: %s (%d)\n", handle, e.what(), e.GetLastError());
			}
		}
	}
};

std::unordered_map<DWORD, BloxxiaProcess> AttachedProcesses;
...
DWORD WINAPI WatchThread(LPVOID)
{
	...
	while (1)
	{
		auto processes = GetBloxxiaProcesses(Settings::UnlockClient, Settings::UnlockStudio);
		...
		for (auto& process : processes)
		{
			...
			if (AttachedProcesses.find(id) == AttachedProcesses.end())
			{
				...
				BloxxiaProcess bloxxia_process;

				bloxxia_process.Attach(process, 3);

				AttachedProcesses[id] = bloxxia_process;
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (UI::IsConsoleOnly)
	{
		...
		{
			Sleep(100);
			process = GetBloxxiaProcess();
		}
		while (!process);

		printf("Found Bloxxia...\n");
		printf("Attaching...\n");

		if (!BloxxiaProcess().Attach(process, 0))
		{
	...
	else
	{
		if (CheckRunning())
		{
			MessageBoxA(NULL, "Bloxxia FPS Unlocker is already running", "Error", MB_OK);
		}
		else
		{
